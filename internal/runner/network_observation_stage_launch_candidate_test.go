...
func TestPrepareNetworkObservationStageLaunchCandidateBindsDestinationAndCredential(t *testing.T) {
	stage, credentials, runtime, _ := networkObservationStageLaunchFixture(t)
	config := submissionStageLaunchCandidateConfig()
	candidate, err := PrepareNetworkObservationStageLaunchCandidate(config, stage, credentials, runtime)
	if err != nil {
		t.Fatal(err)
	}
	...
	again, err := PrepareNetworkObservationStageLaunchCandidate(config, stage, credentials, runtime)
	if err != nil {
		t.Fatal(err)
	}
	againReceipt, _ := again.Receipt()
	if receipt.Format != NetworkObservationStageLaunchCandidateFormat || receipt.State != "PREPARED" || receipt.StageID != "network-observation" || receipt.Authority != "ok-mgmt" || receipt.PreparedAt != "2026-08-16T12:01:00Z" || receipt.ValidUntil != "2026-08-16T12:15:00Z" || receipt.MutationAllowed || receipt.CandidateDigest != againReceipt.CandidateDigest {
		t.Fatalf("unexpected network observation candidate: %#v", receipt)
	}
...
func TestPrepareNetworkObservationStageLaunchCandidateFailsClosed(t *testing.T) {
	stage, credentials, runtime, _ := networkObservationStageLaunchFixture(t)
	...
	for name, mutate := range map[string]func(*SubmissionStageLaunchCandidateConfig){
		...
		t.Run(name, func(t *testing.T) {
			config := valid
			mutate(&config)
			if _, err := PrepareNetworkObservationStageLaunchCandidate(config, stage, credentials, runtime); err == nil {
				t.Fatal("invalid network observation launch candidate accepted")
			}
		})
	}
}
