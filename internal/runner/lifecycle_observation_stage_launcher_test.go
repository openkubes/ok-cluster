...
func TestLifecycleObservationStageLauncherPreflightsThenCreatesJobLast(t *testing.T) {
	...
	launcher := newLifecycleObservationStageLauncher(t, stage, credentials, runtime, api, time.Date(2026, 8, 16, 12, 1, 0, 0, time.UTC))
	...
	if err != nil || receipt.State != "LAUNCHED" || receipt.MutationState != "ATTEMPTED" || len(receipt.Results) != 6 {
		t.Fatalf("lifecycle observation launch failed: %#v %v", receipt, err)
	}
	...
	for _, forbidden := range [][]byte{ledgerToken, observerToken, credentials.objects[0].raw, credentials.objects[1].raw, []byte("short-lived-installer-token"), []byte("created-uid")} {
		if bytes.Contains(public, forbidden) {
			t.Fatal("lifecycle observation receipt exposed private content")
		}
	}
...
func TestLifecycleObservationStageLauncherAcceptsOnlyExactCompleteDuplicate(t *testing.T) {
	stage, credentials, runtime, _, _ := lifecycleObservationStageLaunchFixture(t)
	api := newSubmissionStageLauncherAPI(t)
	now := time.Date(2026, 8, 16, 12, 1, 0, 0, time.UTC)
	first := newLifecycleObservationStageLauncher(t, stage, credentials, runtime, api, now)
	...
	requests := len(api.requests)
	duplicate := newLifecycleObservationStageLauncher(t, stage, credentials, runtime, api, now)
	...
	stage, credentials, runtime, _, _ = lifecycleObservationStageLaunchFixture(t)
	api = newSubmissionStageLauncherAPI(t)
	partial := newLifecycleObservationStageLauncher(t, stage, credentials, runtime, api, now)
	...
	if err == nil || receipt.State != "STOPPED_ZERO_WRITE" || receipt.MutationState != "NOT_ATTEMPTED" || api.posts != 0 || len(api.requests) != 6 {
		t.Fatalf("partial observation state reached mutation: %#v %v", receipt, err)
	}
}

func TestLifecycleObservationStageLauncherPreservesPartialPrefixAndStopsStaleLocally(t *testing.T) {
	stage, credentials, runtime, _, _ := lifecycleObservationStageLaunchFixture(t)
	api := newSubmissionStageLauncherAPI(t)
	api.failPost = 5
	launcher := newLifecycleObservationStageLauncher(t, stage, credentials, runtime, api, time.Date(2026, 8, 16, 12, 1, 0, 0, time.UTC))
	...
	stage, credentials, runtime, _, _ = lifecycleObservationStageLaunchFixture(t)
	api = newSubmissionStageLauncherAPI(t)
	stale := newLifecycleObservationStageLauncher(t, stage, credentials, runtime, api, time.Date(2026, 8, 16, 12, 16, 0, 0, time.UTC))
	...
	if err == nil || receipt.State != "STOPPED_ZERO_WRITE" || receipt.MutationState != "NOT_ATTEMPTED" || len(api.requests) != 0 {
		t.Fatalf("stale observation candidate reached API: %#v %v", receipt, err)
	}
}

func TestOpenLifecycleObservationStageLauncherRequiresExactCandidate(t *testing.T) {
	stage, credentials, runtime, _, _ := lifecycleObservationStageLaunchFixture(t)
	...
	candidate, err := PrepareLifecycleObservationStageLaunchCandidate(candidateConfig, stage, credentials, runtime)
	if err != nil {
		t.Fatal(err)
	}
	...
	config := LifecycleObservationStageLauncherConfig{
	...
	launcher, err := OpenKubernetesLifecycleObservationStageLauncher(config, stage, credentials, runtime)
	if err != nil {
		t.Fatal(err)
	}
	receipt, err := launcher.Launch(context.Background())
	...
	if _, err := OpenKubernetesLifecycleObservationStageLauncher(config, stage, credentials, runtime); err == nil {
		t.Fatal("foreign candidate digest accepted")
	}
}

func newLifecycleObservationStageLauncher(t *testing.T, stage VerifiedLifecycleObservationStagePackage, credentials VerifiedLifecycleObservationStageCredentialPackage, runtime VerifiedLifecycleObservationStageRuntimePrerequisite, api *submissionStageLauncherAPI, now time.Time) *KubernetesLifecycleObservationStageLauncher {
	t.Helper()
	launcher, err := newKubernetesLifecycleObservationStageLauncher(submissionStageLauncherClientConfig{
