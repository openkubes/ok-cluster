...
func validPostPrefixActivationStopCategory(category string) bool {
	...
	case "POST_PREFIX_BINDING_INVALID", "POST_PREFIX_WORKLOAD_AUTHORITY_INVALID",
		...
		"POST_PREFIX_INSTALLER_CREDENTIAL_TRANSPORT_STOPPED", "POST_PREFIX_INSTALLER_CREDENTIAL_CONVERGENCE_EXHAUSTED",
...
func postPrefixInstallerCredentialStopOrFallback(cause error) error {
	var categorized redactedStopCategorizer
	if errors.As(cause, &categorized) {
		switch categorized.RedactedStopCategory() {
		case "POST_PREFIX_OBSERVER_CREDENTIAL_TRANSPORT_STOPPED":
			return newFixedRedactedStop("POST_PREFIX_INSTALLER_CREDENTIAL_TRANSPORT_STOPPED", cause)
		case "POST_PREFIX_OBSERVER_CREDENTIAL_CONVERGENCE_EXHAUSTED":
			return newFixedRedactedStop("POST_PREFIX_INSTALLER_CREDENTIAL_CONVERGENCE_EXHAUSTED", cause)
		case "POST_PREFIX_OBSERVER_CREDENTIAL_RESPONSE_INVALID":
			return newFixedRedactedStop("POST_PREFIX_INSTALLER_CREDENTIAL_RESPONSE_INVALID", cause)
		}
	}
	return newFixedRedactedStop("POST_PREFIX_INSTALLER_CREDENTIAL_ISSUANCE_STOPPED", cause)
}
