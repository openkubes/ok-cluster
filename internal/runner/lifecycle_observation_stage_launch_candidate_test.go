...
func TestPrepareLifecycleObservationStageLaunchCandidateBindsDestinationAndCredential(t *testing.T) {
	stage, credentials, runtime, _, _ := lifecycleObservationStageLaunchFixture(t)
	config := submissionStageLaunchCandidateConfig()
	candidate, err := PrepareLifecycleObservationStageLaunchCandidate(config, stage, credentials, runtime)
	if err != nil {
		t.Fatal(err)
	}
	...
	again, err := PrepareLifecycleObservationStageLaunchCandidate(config, stage, credentials, runtime)
	if err != nil {
		t.Fatal(err)
	}
	againReceipt, _ := again.Receipt()
	if receipt.Format != LifecycleObservationStageLaunchCandidateFormat || receipt.State != "PREPARED" || receipt.StageID != "lifecycle-observation" || receipt.Authority != "ok-mgmt" || receipt.PreparedAt != "2026-08-16T12:01:00Z" || receipt.ValidUntil != "2026-08-16T12:15:00Z" || receipt.MutationAllowed || receipt.CandidateDigest != againReceipt.CandidateDigest {
		t.Fatalf("unexpected lifecycle observation candidate: %#v", receipt)
	}
...
func TestPrepareLifecycleObservationStageLaunchCandidateFailsClosed(t *testing.T) {
	stage, credentials, runtime, _, _ := lifecycleObservationStageLaunchFixture(t)
	...
	for name, mutate := range map[string]func(*SubmissionStageLaunchCandidateConfig){
		...
		t.Run(name, func(t *testing.T) {
			config := valid
			mutate(&config)
			if _, err := PrepareLifecycleObservationStageLaunchCandidate(config, stage, credentials, runtime); err == nil {
				t.Fatal("invalid lifecycle observation launch candidate accepted")
			}
		})
	}
}
